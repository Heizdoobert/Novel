...
describe("adPolicy", () => {
  ...
  describe("sanitizeAdSettingValue", () => {
    it("sanitizes enabled flag to boolean", () => {
      expect(sanitizeAdSettingValue(AD_CONTROL_KEYS.enabled, "true")).toBe(
        true,
      );
      expect(sanitizeAdSettingValue(AD_CONTROL_KEYS.enabled, "false")).toBe(
        false,
      );
    ...
    it("sanitizes minHeight to bounded integer", () => {
      expect(sanitizeAdSettingValue(AD_CONTROL_KEYS.minHeight, "150")).toBe(
        150,
      );
    ...
    it("sanitizes refreshSeconds to bounded integer", () => {
      expect(sanitizeAdSettingValue(AD_CONTROL_KEYS.refreshSeconds, "60")).toBe(
        60,
      );
      expect(sanitizeAdSettingValue(AD_CONTROL_KEYS.refreshSeconds, 5)).toBe(
        15,
      ); // Clamped to min
      expect(sanitizeAdSettingValue(AD_CONTROL_KEYS.refreshSeconds, 7200)).toBe(
        3600,
      ); // Clamped to max
    });
