...
export type RouteRequester =
  { ok: true; id: string; role: string } | { ok: false; status?: number };
...
async function resolveRole(
  ...
  // database lookup is needed — avoiding a per-request profiles query.
  return resolveRequesterRole(userData.user?.app_metadata?.role);
}

async function tryBearerRequester(
  request: NextRequest,
): Promise<RouteRequester | null> {
...
async function trySessionRequester(
  request: NextRequest,
): Promise<RouteRequester | null> {
...
export function isAllowedRouteRole(
  role: string,
  allowedRoles: readonly string[] = DEFAULT_ROUTE_ROLES,
): boolean {
...
export async function resolveRouteRequester(
  ...
  if (
    ...
    internalSecret === process.env.INTERNAL_ADMIN_SECRET
  ) {
  ...
  try {
    ...
    if (options.allowAnonymousFallback) {
      return {
        ok: true,
        id: "anonymous",
        role: options.anonymousRole ?? "employee",
      };
    }
...
export async function requireRouteAuthorization(
  ...
  | { ok: true; requester: { id: string; role: string } }
